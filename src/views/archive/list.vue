  ...
  <div>
    <el-form :inline="true">
      ...
      <el-form-item label="状态" style="min-width: 180px">
        <el-select v-model="table.query.status" clearable width="120px">
  ...
  >
    ...
    >
      ...
      <el-form-item
        ...
        prop="status"
      >
        <el-text type="info"> 存储后添加标签 </el-text>
      </el-form-item>
      <br />
      <el-row :gutter="20">
        <el-col :span="8">
...
const handleRemoteTagList = () => {
  getTag({ name: dialog.value.formTempSelectTag, page: 1, limit: 20 }).then(
    (result) => {
...
const handleRemoteTagSelect = (value) => {
  ...
  addArchiveTag({
    archiveTagId: value.id,
    archiveListId: dialog.value.form.id,
  }).then((result) => {
    const data = {
      id: result.data.id,
      ArchiveTag: value,
...
const handleDeleteTagMap = (id, tags) => {
  ...
  ElMessageBox.confirm(
    "请仔细确认是否删除 " + tags[index].ArchiveTag.name + " ?",
    "警告",
    {
