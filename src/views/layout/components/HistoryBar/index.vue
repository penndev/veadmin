    ...
    >
      <template v-for="(item, key, index) in layoutHistory" :key="index">
        <el-tab-pane v-if="item.meta" :name="key">
          <template #label>
            <span @contextmenu.prevent="openMenu(key, $event)"
              ><i class="hot" />{{ item.meta.title }}</span
            >
          </template>
        </el-tab-pane>
      </template>
    </el-tabs>
    <div ref="veContextMenu" class="ve-contextmenu" @mouseleave="closeMenu">
...
const openMenu = (item, event) => {
  ...
  if (veContextMenu.value.offsetWidth + 5 + event.x > window.innerWidth) {
    veContextMenu.value.style.left =
      window.innerWidth - 5 - veContextMenu.value.offsetWidth + "px";
  } else {
...
const closeLeft = () => {
  ...
  for (const item in layoutHistory.value) {
    if (item === veContextMenuSelect.value) d = true;
    if (d === false && item !== selected.value)
      delete layoutHistory.value[item];
  }
  closeMenu();
};
