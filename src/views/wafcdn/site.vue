    ...
    >
      ...
      >
        <template #default="scope">
          <p
            v-for="item of scope.row.Domains"
            :key="item.id"
          >
            {{ item.name }}
          </p>
    ...
    >
      <el-tabs type="border-card">
        ...
        <el-tab-pane label="安全防护">
          ...
          <el-card
            v-if="dialog.form.security.limit.status"
            shadow="always"
          >
            <el-form-item
              label="流量限速 k/s"
          ...
          <el-card
            v-if="dialog.form.security.sign.status"
            shadow="always"
          >
            <el-form-item
              label="加密密钥"
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              icon="Plus"
              circle
              @click="dialog.form.proxy.header.push({name:null,value:null})"
            />
          </el-card>
          ...
          >
            ...
            <el-button
              ...
              icon="Plus"
              circle
              @click="dialog.form.proxy.cache.push({ruth:null,args: true, time:null, method:[], status:[]})"
            />
          </el-card>
        </el-tab-pane>
      </el-tabs>
  ...
  dialogRef.value.validate((validate) => {
    if (validate) { // 判断表单是否验证通过。
      if (dialog.value.formAction === 'add') {
        postSite(dialog.value.form).then((result) => {
          dialog.value.visible = false
          ElMessage.info('新增完成')
          handleTableData()
        })
      } else if (dialog.value.formAction === 'edit') {
        putSite(dialog.value.form).then((result) => {
          dialog.value.visible = false
          ElMessage.info('修改完成')
          handleTableData()
        })
...
const handleDialogDelete = (row) => {
  ElMessageBox.confirm(`请仔细确认是否删除数据[${row.remark}]?`, '警告', {
    ...
    deleteSite({ id: row.id }).then((result) => {
      ElMessage.info('删除完成')
      handleTableData()
    })
  })
